export class DatabaseService {
    ...
    async addUrl(url, hash, segments) {
        return await this.db.prepare(
            "INSERT INTO urls (url, last_hash, last_clean_text) VALUES (?, ?, ?)"
        ).bind(url, hash, JSON.stringify(segments)).run();
    }

    async updateUrl(id, hash, segments) {
        return await this.db.prepare(
            "UPDATE urls SET last_hash = ?, last_clean_text = ?, error_count = 0 WHERE id = ?"
        ).bind(hash, JSON.stringify(segments), id).run();
    }
    ...
    async logChange(urlId, summary) {
        return await this.db.prepare("INSERT INTO changes_log (url_id, change_summary) VALUES (?, ?)").bind(urlId, summary).run();
    }

    async logError(urlId, message, type = 'general', level = 'warning') {
        return await this.db.prepare(
            "INSERT INTO errors (url_id, error_message, type, level) VALUES (?, ?, ?, ?)"
        ).bind(urlId, message, type, level).run();
    }
    ...
    async logTelemetry(stats) {
        ...
        ).bind(stats.urls_checked, stats.changes_found, stats.llm_calls, stats.notifications_sent, stats.errors_count, stats.duration_ms).run();
    }
    ...
    async findSimilarEvent(date_iso, title) {
        ...
        // 2. Check fuzzy similarity OR Substring match
        for (const event of results) {
            const t1 = title.toLowerCase();
            const t2 = event.title.toLowerCase();

            // Check if one includes the other (e.g. "Event Name" vs "Event Name (Details)")
    ...
    async addEvent(event) {
        return await this.db.prepare(
            "INSERT INTO events (url_id, title, summary, description, date_iso, price_info, location, source_link, hash, is_future) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        ).bind(event.url_id, event.title, event.summary, event.description, event.date_iso, event.price_info, event.location, event.source_link, event.hash, event.is_future).run();
    }
    ...
    async addDiscoveredUrl(sourceUrlId, url, title, context) {
        ...
        ).bind(sourceUrlId, url, title, context).run();
    }
    ...
    async getFutureEvents() {
        ...
        // Show events from the last 30 days to include recent/ongoing ones
        return await this.db.prepare(
